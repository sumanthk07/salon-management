...
<body>
    ...
    <?php
            ...
            $query = mysqli_query($conn,"SELECT C.C_FNAME, C.C_LNAME, C.C_ADD,B.INV_NUM,B.APP_ID,B.INV_DATE,B.PAID_DATE,B.PAID_STATUS,MAX(B.AMT_PAYABLE),A.SER_NAME 
                      ...
                      WHERE B.APP_ID=A.APP_ID AND A.AC_ID=C.C_ID AND B.APP_ID=$APP_ID;");
        
				while ($row = mysqli_fetch_assoc($query))
                {
                echo"<br>";
                echo'<h2 align="right"> Date:'.$row['INV_DATE'].'</h2>';
                echo '<h2>Customer Name:' .$row['C_FNAME']. ' &nbsp' .$row['C_LNAME'].'</h2>';
    ...
    <div align= "center" ;>
        <br>
                    <button style="height:40px; width:80px;" onclick="window.print();">Print</button>	
    </div>
    </body>
</html>
